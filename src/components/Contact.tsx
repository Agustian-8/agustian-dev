    ...
    <section id="kontak" className="py-24 px-6 max-w-6xl mx-auto">
      {showSuccess && (
        <motion.div
          ...
          className="fixed top-24 left-1/2 -translate-x-1/2 z-50 bg-[#6B7B4A] text-white px-6 py-3 rounded-full shadow-lg flex items-center gap-2 text-sm"
        >
          <CheckCircle size={18} /> WhatsApp terbuka! Lanjutkan chat di sana 🚀
        </motion.div>
      )}
      ...
      >
        <h2 className="text-3xl md:text-4xl font-bold text-[#2D3A1F] mb-4">
          Mari <span className="text-[#576238]">Berkolaborasi</span>
        </h2>
        <p className="text-[#5A6B3E]">Punya ide proyek atau butuh bantuan teknis? Kirim pesan dan mari kita diskusikan.</p>
      ...
      <div className="grid md:grid-cols-5 gap-8">
        ...
        >
          <div className="p-6 bg-[#6B7B4A]/5 rounded-2xl border border-[#576238]/10">
            <h3 className="font-bold text-[#2D3A1F] flex items-center gap-2 mb-4">
              <FaWhatsapp className="text-[#6B7B4A]" /> Hubungi Saya
            </h3>
            <p className="text-[#5A6B3E] text-sm mb-6">
              Isi form & langsung diarahkan ke WhatsApp. Respon cepat!
            </p>
            ...
            {[
              ...
              { icon: <MapPin size={18} />, label: "Lokasi", value: "Pontianak, Kalimantan Barat", type: null },
            ].map((item, i) => (
              <div key={i} className="flex items-center justify-between p-4 bg-[#E8E0D0]/30 rounded-xl border border-[#576238]/10 mb-3">
                ...
                {item.type && (
                  <button onClick={() => copyToClipboard(item.type === "wa" ? WHATSAPP : EMAIL, item.type)} className="p-2 hover:bg-[#D0C8B8]/50 rounded-lg transition-all">
            ...
            <div className="flex items-center gap-2 p-3 bg-[#E8E0D0]/20 rounded-xl mt-4">
              ...
              <span className="text-xs text-[#5A6B3E]">Online — respon dalam hitungan menit</span>
            </div>
          </div>
        </motion.div>
        ...
        >
          <form onSubmit={handleSubmit} className="p-6 md:p-8 bg-[#E8E0D0]/20 backdrop-blur-sm rounded-2xl border border-[#576238]/10 space-y-5">
            <div className="flex items-center gap-3 pb-4 border-b border-[#576238]/10">
              ...
              <div>
                <h3 className="font-bold text-[#2D3A1F]">Kirim Pesan</h3>
                <p className="text-xs text-[#5A6B3E]">Via WhatsApp</p>
              </div>
            </div>

            <div className="grid md:grid-cols-2 gap-4">
              <input type="text" name="name" value={formData.name} onChange={handleChange} required placeholder="Nama Lengkap" className="w-full px-4 py-3 bg-[#F0EADC]/50 border border-[#576238]/20 rounded-xl text-[#2D3A1F] placeholder:text-[#7D8F5A] focus:outline-none focus:border-[#6B7B4A] transition-all" />
            ...
            <textarea name="message" value={formData.message} onChange={handleChange} required rows={4} placeholder="Ceritakan proyek Anda..." className="w-full px-4 py-3 bg-[#F0EADC]/50 border border-[#576238]/20 rounded-xl text-[#2D3A1F] placeholder:text-[#7D8F5A] focus:outline-none focus:border-[#6B7B4A] transition-all resize-none" />
            ...
            <button type="submit" className="w-full flex items-center justify-center gap-3 bg-gradient-to-r from-[#6B7B4A] to-[#7D8F5A] hover:from-[#576238] hover:to-[#6B7B4A] text-white font-semibold py-4 rounded-xl shadow-lg hover:shadow-[#6B7B4A]/20 transition-all">
              <FaWhatsapp size={20} /> Lanjutkan ke WhatsApp <Send size={16} />
            </button>
          </form>
        </motion.div>
      </div>
